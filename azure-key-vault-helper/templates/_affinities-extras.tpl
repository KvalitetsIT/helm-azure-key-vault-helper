...
preferredDuringSchedulingIgnoredDuringExecution:
  - podAffinityTerm:
      labelSelector:
        matchLabels: {{- (include "azure-key-vault-helper.selectorLabels-extras" (dict "root" .context "name" .name)) | nindent 10 }}
...
requiredDuringSchedulingIgnoredDuringExecution:
  - labelSelector:
      matchLabels: {{- (include "azure-key-vault-helper.selectorLabels" .context) | nindent 8 }}
        {{- range $key, $value := $extraMatchLabels }}
        {{ $key }}: {{ $value | quote }}
        {{- end }}
    namespaces:
